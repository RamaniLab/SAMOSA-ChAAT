#Now we're going to process all of the unremodeled vs remodeled fibers
#and compute autocorrelograms
...
#Iterate through each density, and cluster -- first for S1
fho = open('S1_remodeled_leiden_autocor_labels.data', 'w')
subSample1 = sampleRef[(sampleRef['seq'] == 'S1') & (sampleRef['density'] != 'Negative') & 
                       (sampleRef['turnover'] == 'Single-turnover') & 
                       ((sampleRef['catalytic'] == 'Native')|(sampleRef['catalytic'] == '(+)ATP'))]
...
for i in range(len(cell_names)):
    ...
    density_csv = pd.read_csv('/avicenna/vramani/analyses/pacbio/%s/processed/density/%s_%s_density.csv' % 
        (cell_name, cell_name, samp_name))
...
for i in range(len(mols)):
    molcor = ccf(mols[i], mols[i])
    cors.append(molcor)
autocors = np.vstack(cors)
clusters, um_x, um_y = cluster_mols(autocors)
for i in range(len(clusters)):
    print("%s\t%s\t%s\t%s\t%s" % (labels[i], clusters[i], um_x[i], um_y[i], densities[i]), file = fho) 
fho.close()

#Second for S2 fibers

fho = open('S2_remodeled_leiden_autocor_labels.data', 'w')
subSample1 = sampleRef[(sampleRef['seq'] == 'S2') & (sampleRef['density'] != 'Negative') & 
                       (sampleRef['turnover'] == 'Single-turnover') & 
                       ((sampleRef['catalytic'] == 'Native')|(sampleRef['catalytic'] == '(+)ATP'))]
cell_names = subSample1['cell'].values
samp_names = subSample1['sampleName'].values
...
# fho = open('S2_remodeled_leiden_umap.data', 'w')
# for density in np.unique(sampleRef['density']):
#     if density != 'Negative':
#         subSample1 = sampleRef[(sampleRef['seq'] == 'S2') & (sampleRef['density'] == density)]
#         cell_names = subSample1['cell'].values
#         samp_names = subSample1['sampleName'].values
#         samp_labels = subSample1['sample_label'].values
#         mols = []
