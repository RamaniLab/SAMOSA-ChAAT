...
def main():
    ...
    mdata.to_csv('KOvWT_access_ch12_final.data',sep=',')
    ...
    mdata_labels.to_csv('KOvWT_access_chr12_sep_labels.data',sep=',')
    #Save mols matrix for plotting, clustering, etc.
#    np.save('othersites_mols_%sbp.npy' % length, mats)
    np.save('chr12sites_mols_%sbp.npy' % length, mats)
